...
exports.kill = (id, stateFuncs, senderFunc, age) => {
  ...
  image.getImage(celeb.name).then((imgPath) => {
    const imageEmbed = new MessageEmbed()
      .setImage(imgPath)
      .setDescription(celeb.name);
    ...
    senderFunc({ embeds: [imageEmbed] }).then(() => {
      if (winners.length > 0) {
        ...
        const winnerString = format.stringCommaList(
          winners.map((winner) => currentState.players[winner].name),
        );
      ...
      if (losers.length > 0) {
        const loserString = format.stringCommaList(
          losers.map((loser) => currentState.players[loser].name),
        );
        senderFunc(
          `Commiserations to ${loserString}, who considered this pick in the past before changing their mind.`,
        );
      }
    });
  });
