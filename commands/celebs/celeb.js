...
module.exports = {
  ...
  execute(msg, args, stateFuncs) {
    ...
    image.getImage(state.celebs[celeb].name).then((imgPath) => {
      let output =
        "Status: " + (state.celebs[celeb].isAlive ? "Alive" : "Dead");
      if (players.length > 0) {
        output =
          output +
          "\nPicked by: " +
          format.stringCommaList(
            players.map((player) => state.players[player].name),
          ) +
          ".";
      }
      ...
      const imageEmbed = new MessageEmbed()
        .setImage(imgPath)
        .setDescription(state.celebs[celeb].name);

      console.log("[celeb]: ", imgPath);

      let nameHeader = format.bold(state.celebs[celeb].name);
      msg.channel.send(
        (state.celebs[celeb].isAlive
          ? nameHeader
          : format.strikethrough(nameHeader)) +
          " " +
          format.italic(`(ID: '${celeb}')`),
      );
