  ...
  return {
    ...
    execute(msg, args, stateFuncs) {
      if (new Date() > PICK_CUTOFF_DATE) {
        msg.reply(
          `No adjustments to picks are accepted after ${PICK_CUTOFF_DATE.toDateString()}.`
        );
        return;
      }

      if (!args || !args[0]) {
        error.usage(`${command} [name]`, msg);
        return;
      }
      ...
      if (!playerId) {
        msg.channel.send(
          `Unable to find player ${format.bold(msg.author.username)}.`
        );
        return;
      }
      ...
      if (!celeb) {
        msg.channel.send(
          `${format.bold(
            celebName
          )} isn't on your list. Or mine for that matter.`
        );
        return;
      }
      ...
      msg.channel.send(
        `${format.bold(state.players[playerId].name)} has removed ${format.bold(
          celebName
        )} from their list.`
      );
    },
