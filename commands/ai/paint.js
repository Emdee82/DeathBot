...
module.exports = {
  name: "!paint",
  description:
    "Generates an image using the configure AI model and describes the output.",
  async execute(msg, args, stateFuncs) {
    if (+OPENAI_ENABLED) {
      ...
      console.log(
        new Date(),
        `[paint]: ${msg.author.username} has asked DeathBot to paint:`,
        prompt,
      );
      const state = stateFuncs.getState();
      var sender =
        state.playerKeys.filter(
          (x) => msg.author.id == state.players[x].userId,
        )[0] || msg.author.username;
      var senderString = format.sentenceCase(sender);
      state.chatMessages = stateFuncs.addMessage(
        "user",
        (sender || "Someone") +
          " has asked DeathBot to paint the following picture: " +
          prompt,
      );
      ...
      try {
        ...
        state.chatMessages = stateFuncs.addMessage(
          "system",
          `Using a separate AI image generation API call, you 'painted' for ${sender} the following: ${response.data[0].revised_prompt}`,
        );
        msg.reply(
          `${format.bold("DeathBot")} has painted for ${senderString}:\n${response.data[0].revised_prompt}`,
        );
      } catch (error) {
        msg.reply(
          `Unable to paint something like "${prompt}" for ${senderString} - ${error.error}`,
        );
      }
    } else {
