...
const isRestrictedCommand = (cmd, msg) => {
  ...
  if (!user || cmd.restrictionLevel < user.restrictionLevel) {
    console.error(
      `Restricted command - ${msg.author.username} attempted to invoke`,
      cmd
    );
    return true;
  }
...
bot.once("ready", () => {
  console.log(
    new Date(),
    `[death-bot]: Bot logged in under username ${bot.user.tag}`
  );
  ...
  console.log(
    new Date(),
    "[death-bot]: Channel connections established successfully."
  );
...
bot.on("error", (err) => {
  console.error(
    new Date(),
    "[death-bot]: An unknown error occurred - details follow:",
    err
  );
});
